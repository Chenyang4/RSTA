...
from lib.augmentation import denormalization
from lib.gmm import get_gmm
from lib.utils import utils, lr_scheduler
from lib.models import build_model
from lib.losses import non_saturating_loss
from lib.losses.label_smooth import LabelSmoothing, SampleLabelSmoothing
...
def main(args):
    ...
    train_loader = DataLoader(train_data, args.batch_size, not args.dist, sampler,
                              ...
                              drop_last=True)
    eval_loader = DataLoader(eval_data, args.batch_size, not args.dist, sampler,
                             num_workers=args.num_workers, pin_memory=True)
    # Model
    ...
    # prepare for ReSmooth
    if args.gmm or args.ls:
        model_res = build_model(args.model, n_classes).to(device)
        optim_res = optim.SGD(model_res.parameters(), lr=args.lr, momentum=0.9, nesterov=True, weight_decay=args.weight_decay)
        scheduler_res = lr_scheduler.CosineAnnealingWithLinearWarmup(optim_res, 5, args.n_epochs)
        criterion_org = LabelSmoothing(smoothing=args.smooth_org).to(device)
        criterion_aug = SampleLabelSmoothing(smoothing=args.smooth_aug).to(device) if args.gmm \
            else LabelSmoothing(smoothing=args.smooth_aug).to(device)
    ...
    for epoch in range(st_epoch, args.n_epochs + 1):
        ...
        if args.gmm:
            gmm = get_gmm(args.model, n_classes, train_loader, args.pretrained, device, copy.deepcopy(objective))
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.gmm or args.ls:
                n_ori = int(inputs.size(0) * args.prob)
                inputs_aug = torch.stack([base_aug(_x) for _x in aug_img]) if base_aug is not None else aug_img
                inputs = objective.normalizer(inputs) if objective.normalizer is not None else inputs
                data_res = torch.cat([inputs[:n_ori], inputs_aug[n_ori:]], dim=0)

                preds_res = model_res(data_res)

                if args.gmm:
                    prob = torch.ones_like(targets, dtype=torch.float)
                    prob[n_ori:] = torch.tensor(gmm.predict(data_res[n_ori:], targets[n_ori:]), device=targets.device) \
                        if len(data_res[n_ori:]) else prob[n_ori:]
                    loss_aug = criterion_aug(preds_res[n_ori:], targets[n_ori:], prob[n_ori:]) if len(preds_res[n_ori:]) else torch.tensor(0., device=device)
                else:
                    loss_aug = criterion_aug(preds_res[n_ori:], targets[n_ori:]) if len(preds_res[n_ori:]) else torch.tensor(0., device=device)
                loss_org = criterion_org(preds_res[:n_ori], targets[:n_ori]) if len(preds_res[:n_ori]) else torch.tensor(0., device=device)
            ...
            if args.gmm or args.ls:
                scheduler_res.step(epoch - 1. + (i + 1.) / len(train_loader))
        ...
        if args.gmm or args.ls:
            writer.add_image('train/1.origin_image', denormalization(inputs[0]), epoch)
            writer.add_image('train/2.augmented_image', denormalization(inputs_aug[0]), epoch)
        else:
            writer.add_image('train/1.origin_image', inputs[0], epoch)
            writer.add_image('train/2.augmented_image', aug_img[0], epoch)
        ...
        if main_process:
            ...
            with torch.no_grad():
                for data in eval_loader:
                    ...
                    acc1 += accs[0] * output.size(0)
                    acc5 += accs[1] * output.size(0)
                    n_samples += output.size(0)
            logger.info(f'evaluation: {args.dataset} error rate (%) | Top1 {100 - acc1 / n_samples} | Top5 {100 - acc5 / n_samples}')
            ...
            if args.gmm or args.ls:
                ...
                with torch.no_grad():
                    for data in eval_loader:
                        input, target = data
                        output = model_res(input.to(device))
                        accs = utils.accuracy(output, target.to(device), (1, 5))
                        acc1 += accs[0] * output.size(0)
                        acc5 += accs[1] * output.size(0)
                if args.gmm:
                    logger.info(f'resmooth: {args.dataset} error rate (%) | Top1 {100 - acc1 / n_samples} | Top5 {100 - acc5 / n_samples}')
                else:
                    logger.info(f'ls: {args.dataset} error rate (%) | Top1 {100 - acc1 / n_samples} | Top5 {100 - acc5 / n_samples}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--smooth-aug', default=0.3, type=float, help='smooth for augmentation')
    ...
    writer = SummaryWriter(log_dir='%s' % args.log_dir)
